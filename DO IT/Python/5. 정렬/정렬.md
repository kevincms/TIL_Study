...
> 4. [퀵 정렬](#quick-sort)
>> - 하나의 축을 정하여 그 축을 기준으로 왼쪽과 오른쪽을 분리하여 정렬.
>> - 축이 중앙에 있는 경우와 끝에 있는 경우를 하나의 코드로 어떻게 정리해야 될지 아직 파악 못함.
>> - 그냥 재귀함수로 축(Pivot)과 축보다 작은 배열과 큰 배열을 나누기로 함.
>> - 다만 이런 경우 공간복잡도가 커지긴 함.
...
|---|---:|---:|---:|---:|---:|---:| 
|최선 시간 복잡도|O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(n)|O(logn)|2|2|  
|평균 시간 복잡도|O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(nlogn)|2|2|  
|최악 시간 복잡도|O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(n<sup>2</sup>)|O(n<sup>2</sup>)|2|2|  
|공간 복잡도|O(N)|O(N)|O(N)|O(N)|2|2|  
